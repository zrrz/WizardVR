...
public class PKFxFXEditor : Editor
{
	...
	private SerializedProperty SamplerField(SerializedProperty sampler)
	{
		...
		if (m_Type.intValue == (int)PKFxManager.ESamplerType.SamplerShape)
		{
			...
			m_ShapeType.intValue = EditorGUILayout.Popup(m_ShapeType.intValue + 1, ShapeTypes);
			m_ShapeType.intValue -= 1;
			if (m_ShapeType.intValue == (int)PKFxManager.EShapeType.BoxShape)
				BoxField(sampler);
			else if (m_ShapeType.intValue == (int)PKFxManager.EShapeType.SphereShape)
				SphereField(sampler);
			...
			else if (m_ShapeType.intValue == (int)PKFxManager.EShapeType.MeshShape)
				MeshField(sampler);
			else if (m_ShapeType.intValue == (int)PKFxManager.EShapeType.SkinnedMeshShape)
	...
	private static readonly string[] ShapeTypes = { "None", "BOX", "SPHERE", "CYLINDER", "CAPSULE", "MESH", "SKINNEDMESH" };
